...
def get_snp_store(data_path=data_path):
...
# data_path is changeable depending on desired save location
def get_data(tickers, start='2000-01-01', data_path=data_path, get_new=False):
    interval = '1d'
    prefix = interval + '/'
...
def build_1mo(ticker, data_path=data_path):
    if '.csv' in ticker:
...
def build_data(interval, data_path=data_path):
...
# data_path is changeable depending on desired save location
def check_ticker(ticker, offset, interval = '1mo', data_path=data_path):
    prefix = interval + '/'
    try:
...
# data_path is changeable depending on desired save location
def build_portfolio(tickers, hist_depth, train_depth, start='2000-01-01', interval='1mo', data_path=data_path, offset=True, get_new=False):
    if type(offset) == bool:
